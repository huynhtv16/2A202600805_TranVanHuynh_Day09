...
## Tối ưu theo bài học Stage 5 (Distributed A2A)

Dựa trên bài hôm nay, bạn có thể tối ưu lab assignment bằng cách tách pipeline thành các module độc lập và thiết kế chúng giống một hệ thống multi-agent:

- **Phân tách rõ ràng:**
  - `task1/2/3` là dữ liệu đầu vào
  - `task4` là indexing
  - `task5/6/7/8/9` là retrieval
  - `task10` là generation
- **Độc lập và có interface rõ ràng:** Mỗi task nên nhận input và trả về output dạng JSON/metadata để dễ chuyển sang service riêng nếu cần.
- **Quản lý metadata:** Giữ `source`, `chunk_id`, `score`, `model`, `timestamp` trong mỗi result để dễ trace và debug.
- **Fallback & orchestration:** Thiết kế `retrieve()` như một coordinator, tương tự `Customer Agent` trong Stage 5:
  - semantic search → lexical search → merge/rerank → fallback vectorless
- **Dynamic discovery concept:** Nếu sau này mở rộng, bạn có thể cho mỗi module thành microservice và để một service tổng điều phối các bước, giống `Registry + Customer Agent`.

> Mục tiêu: làm cho lab assignment không chỉ chạy được, mà còn dễ bảo trì, dễ kiểm thử, và dễ mở rộng thành kiến trúc phân tán.
...
**Trạng thái nhóm:**
- [x] Giao diện chat Streamlit đã triển khai
- [x] Câu trả lời có citation / sources
- [x] Follow-up questions / history lưu trên `st.session_state`
- [x] Hiển thị tài liệu nguồn từ 2 bộ dữ liệu
...
User Query
  ...
  └─→ Nếu score thấp → PageIndex Vectorless (Task 8)
        ↓
    Top Documents → Generation with citation (Task 10)
        ↓
    Streamlit Chatbot (streamlit_app.py)
```

**Mô tả:**
- **Data Layer:** các file markdown chuẩn hoá trong `data/standardized/`
- **Retriever Layer:** kết hợp semantic + lexical search, fusion và reranking
- **Fallback Layer:** PageIndex vectorless khi hybrid search không đủ
- **Generation Layer:** trả lời có citation hoặc extraction fallback
...
|-----------|------|----------|------------|
| Trần Văn Huỳnh | 2A202600805 | Triển khai toàn bộ sản phẩm nhóm: Streamlit RAG chatbot, retrieval + generation, evaluation pipeline, demo và docs | Hoàn thành |
